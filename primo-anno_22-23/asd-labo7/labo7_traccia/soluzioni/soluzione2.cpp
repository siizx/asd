...
{
   if (isEmpty(t))
      return false; // se t e' vuoto restituisco false
...
{
   list::List lst = list::createEmpty();
   if (father(l, t) == emptyLabel || isEmpty(t))
      return lst;
...
Label tree::leastCommonAncestor(const Label label1, const Label label2, const Tree &t)
{
   ...
   if (list::isEmpty(lst1) || list::isEmpty(lst2))
      return emptyLabel; // se una delle due liste di ancestor e' vuota, non c'e' antenato comune
